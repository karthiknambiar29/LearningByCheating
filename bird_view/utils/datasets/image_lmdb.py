...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        rgb_image_left = np.fromstring(lmdb_txn.get(('rgb_left_%04d'%index).encode()), np.uint8).reshape(600,800,3) # (160,384,3)
        rgb_image_right = np.fromstring(lmdb_txn.get(('rgb_right_%04d'%index).encode()), np.uint8).reshape(600,800,3) # (160,384,3)
